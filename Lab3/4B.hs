...
Value : Entry in Sudokou ranges from 0 to 9 where 0 means empty
...
checkRepeat sudoku (pos:xs)
        ...
        | otherwise = do
                    ...
                    if (isValueValid sudokuTemp pos val) then 
                        ...
                        False
{-
This function checks if the input list of list is of 9 rows
-}
checkNoOfRows :: [[Value]] -> Bool
checkNoOfRows x = (length x == 9)  

{-
This function checks if all the rows contains 9 columns each row is valid
-}
checkAllValues :: [[Value]] -> Bool
checkAllValues [] = True
checkAllValues (x:xs)
            | (length x == 9 && checkRowValues x) = checkAllValues xs
            | otherwise = False

{-
This function checks the row to validate each element present is between 0 to 9
-}           
...
tuple of (Position,Value) by creating each cell from (0,0) to (8,8)
-}
